...
pub(crate) fn parse_search_object(
    ...
    if let Some(order_by_recency) = search_object.get("order_by_recency").and_then(|v| v.as_bool()) {
    ...
    if let Some(next_batch) = search_object.get("next_batch").and_then(|v| v.as_str()) {
        let token = next_batch.parse::<Uuid>()
...
) -> Result<Option<CrawlerCheckpoint>> {
    match checkpoint {
        Some(Value::Object(obj)) => {
            let room_id = obj.get("roomId")
                .and_then(|v| v.as_str())
            ...
            let full_crawl = obj.get("fullCrawl")
...
pub(crate) fn parse_event(
    ...
    let sender = event_obj.get("sender")
        .and_then(|v| v.as_str())
        .context("Missing sender")?
        .to_string();

    let event_id = event_obj.get("event_id")
        .and_then(|v| v.as_str())
        .context("Missing event_id")?
        .to_string();

    let server_timestamp = event_obj.get("origin_server_ts")
        .and_then(|v| v.as_f64())
        .context("Missing or invalid timestamp")? as i64;

    let room_id = event_obj.get("room_id")
        .and_then(|v| v.as_str())
    ...
    let event_type_str = event_obj.get("type")
        .and_then(|v| v.as_str())
    ...
    let msgtype = match event_type {
        EventType::Message => content.get("msgtype")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string()),
        _ => None,
    };

    let event_source = serde_json::to_string(event)
        .context("Cannot serialize event")?;
...
pub(crate) fn parse_profile(
    ...
    let displayname = profile_obj.get("displayname")
        .and_then(|v| match v {
    ...
    let avatar_url = profile_obj.get("avatar_url")
        .and_then(|v| match v {
            Value::String(s) => Some(s.to_string()),
            Value::Null => None,
            _ => None,
...
) -> Result<(Vec<(Event, Profile)>, Option<CrawlerCheckpoint>, Option<CrawlerCheckpoint>)> {
    let mut parsed_events: Vec<(Event, Profile)> = Vec::new();

    for event_obj in events {
        let event_obj = event_obj.as_object()
            .context("Event must be an object")?;

        let event = parse_event(&Value::Object(event_obj.clone()))?;

        let profile = event_obj.get("profile")
...
pub(crate) fn search_result_to_json(
    ...
    context.insert("events_before".to_string(), serde_json::Value::Array(before));
    context.insert("events_after".to_string(), serde_json::Value::Array(after));
    context.insert("profile_info".to_string(), serde_json::Value::Object(profile_info));
...
pub(crate) fn search_result_to_serialized(
    result: SearchResult
) -> Result<String> {
...
pub(crate) fn deserialize_event(
    source: &str,
